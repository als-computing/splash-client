        ...
        <b-row>
          ...
          <b-col lg="9">
            <b-card>
              ...
              <h4
                v-if="compound.documentation.sections.length == 0"
              >No documentation for this compound. Be the first to add some!</h4>
...
export default {
  ...
  mounted() {
    // clone array of objects and their primitive properties
    // nested objects and arrays will not be copied
    // https://stackoverflow.com/a/40283265/8903570
    this.fields = this.fieldsParent.map((field) => ({ ...field }));
    this.mounted = true;
    // console.log('mounted');
  },
  methods: {
    ...
    async saveEdit(indexChanged, originalSection) {
      try {
        this.saving = true;
        this.compound.documentation.sections[indexChanged] = {
