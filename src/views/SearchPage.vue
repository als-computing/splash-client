<template>
    <div class = "search-page">
            ...
            <table
            class="table-responsive table-borderless table-striped table results"
            v-show="searchResults.length!==0">
            <thead v-if="searchResults.length!=0">
                <tr>
                <th scope="col">Experiment</th>
                <th scope="col">Researcher</th>
                <th scope="col">Polymer</th>
                <th scope="col">Solutes Present</th>
            ...
            <tbody>
                ...
                :key=i
                class="search-result">
                <td>{{result.experimentName}}</td>
                <td>{{result.researcherName}}</td>
                <td>{{result.polymer}}</td>
                <td><span v-for="(solute, i) in result.solutesPresent" v-bind:key ="i">{{ solute }}, </span></td>
                <td>{{result.researcherGroup}}</td>
                <td>{{result.researcherInstitution}}</td>
                </tr>
            </tbody>
            </table>
            <div class ="footer navbar fixed-bottom justify-content-center">
            <b-pagination-nav v-if="totalPages > 1 && currPageAsync != 0" :link-gen="linkGen" :number-of-pages="totalPages" :value="currPageAsync" use-router>
            </b-pagination-nav>
            </div>
    </div>
</template>
...
export default {
  ...
  computed: {
    ...
    currPageComputed() {return this.$route.query.page;}
  },
  methods: {
    linkGen(pageNum) {
      return {
    ...
    search() {
      ...
      if (!this.$route.query.query) this.$router.replace('/');
        ...
        if (!this.currPageComputed || !Number.isInteger(Number(this.currPageComputed)) || Number(this.currPageComputed) <= 0) {
          this.$router.replace({ path: 'search', query: { query: this.$route.query.query, page: 1 } });
        }
        ...
        axios.post('/research_experiments/_search',
          {
            ...
            query: {
              multi_match: {
                query: this.$route.query.query,
                fields: ['name', 'researcher.name', 'researcher.group', 'researcher.institution', 'trials.solutes_present', 'trials.membrane_or_polymer'],
          ...
          this.$nextTick().then(() => {this.currPageAsync = this.$route.query.page;})
          this.searchResults = [];
            ...
            return axios.post('/research_experiments/_search',
              {
                ...
                query: {
                  multi_match: {
                    query: this.$route.query.query,
                    fields: ['name', 'researcher.name', 'researcher.group', 'researcher.institution', 'trials.solutes_present', 'trial.membrane_or_polymer'],
                    fuzziness: 'AUTO',
                    minimum_should_match: '3<90%',
                  },
                },
